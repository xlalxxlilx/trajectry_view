...
- Chrome / Edge での `file://` 直開きは CORS 制約で動作しません
- 必ず `http://localhost` などの静的サーバー経由で開いてください

最小手順 (Windows):

1. プロジェクト直下に移動
2. `py -m http.server 8080` を実行
3. ブラウザで `http://127.0.0.1:8080/web_pyodide/index.html` を開く
